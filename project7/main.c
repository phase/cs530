        ...
        if (strcmp(str, "quadric") == 0) {
            ...
            while (ch != '\n' && ch != EOF) {
                ...
                if (strcmp(item, "diffuse_color") == 0) {
                    fgetc(fh); // skip space
                    fscanf(fh, "[%f, %f, %f], ", &obj->diffuse_color[0], &obj->diffuse_color[1],
                           &obj->diffuse_color[2]);
                } else if (strcmp(item, "specular_color") == 0) {
                    fgetc(fh); // skip space
                    fscanf(fh, "[%f, %f, %f], ", &obj->specular_color[0], &obj->specular_color[1],
                           &obj->specular_color[2]);
...
float findQuadricIntersection(Ray ray, Object *obj, float *hitDest) {
    ...
    float rx, ry, rz;
    float Aq = obj->quadric->a * powf(ray.unitRay[0], 2) + obj->quadric->b * powf(ray.unitRay[1], 2) +
               obj->quadric->c * powf(ray.unitRay[2], 2) +
               obj->quadric->d * ray.unitRay[0] * ray.unitRay[1] + obj->quadric->e * ray.unitRay[0] * ray.unitRay[2] +
               obj->quadric->f * ray.unitRay[1] * ray.unitRay[2];
    float Bq = 2 * obj->quadric->a * ray.position[0] * ray.unitRay[0] +
               2 * obj->quadric->b * ray.position[1] * ray.unitRay[1] +
               2 * obj->quadric->c * ray.position[2] * ray.unitRay[2] +
               obj->quadric->d * (ray.position[0] * ray.unitRay[1] + ray.position[1] * ray.unitRay[0]) +
               obj->quadric->e * (ray.position[0] * ray.unitRay[2] + ray.position[2] * ray.unitRay[0]) +
               obj->quadric->f * (ray.position[1] * ray.unitRay[2] + ray.position[2] * ray.unitRay[1]) +
               obj->quadric->g * ray.unitRay[0] + obj->quadric->h * ray.unitRay[1] + obj->quadric->i * ray.unitRay[2];
    float Cq = obj->quadric->a * powf(ray.position[0], 2) + obj->quadric->b * powf(ray.position[1], 2) +
               obj->quadric->c * powf(ray.position[2], 1) +
               obj->quadric->d * ray.position[0] * ray.position[1] +
               obj->quadric->e * ray.position[0] * ray.position[2] +
               obj->quadric->f * ray.position[1] * ray.position[2] +
               obj->quadric->g * ray.position[0] + obj->quadric->h * ray.position[1] +
               obj->quadric->i * ray.position[2] + obj->quadric->j;
        ...
        if (Object_flag == 1) {
            if (obj->flag == 0 && !(v3_equals(obj->position, shooter->position, 0.01))) {
                ...
                distance = findPlaneIntersection(ray, obj, hit);
                       ...
                       (obj->quadric->a != shooter->quadric->a || obj->quadric->b != shooter->quadric->b ||
                        obj->quadric->c != shooter->quadric->c || obj->quadric->d != shooter->quadric->d ||
                        obj->quadric->e != shooter->quadric->e || obj->quadric->f != shooter->quadric->f ||
                        obj->quadric->g != shooter->quadric->g || obj->quadric->h != shooter->quadric->h ||
                        obj->quadric->i != shooter->quadric->i || obj->quadric->j != shooter->quadric->j)) {
...
Image *render(Scene scene, int imageWidth, int imageHeight) {
    ...
    float redCenter[3] = {0.03f, 0.3f, -1.0f};
    float blueCenter[3] = {-0.03f, 0.3f, -1.0f};

    Image *image = newImage(imageWidth, imageHeight, 255);

    for (int i = 0; i < imageHeight; i++) { // for each row
        float pyr = redCenter[1] - scene.height / 2.0f + pixelHeight * ((float) i + 0.5f); // y coord of row
        float pyb = blueCenter[1] - scene.height / 2.0f + pixelHeight * ((float) i + 0.5f); // y coord of row
        for (int j = 0; j < imageWidth; j++) { // for each column
            float pxr = redCenter[0] - scene.width / 2.0f + pixelWidth * ((float) j + 0.5f); // x coord of column
            float pxb = blueCenter[0] - scene.width / 2.0f + pixelWidth * ((float) j + 0.5f); // x coord of column
            float pzr = redCenter[2];
            float pzb = blueCenter[2];

            float pr[3] = {pxr, pyr, pzr};
            float pb[3] = {pxb, pyb, pzb};

            // two phases for red and then blue+green
            for (int phase = 0; phase < 2; phase++) {
                float *p = phase == 0 ? pr : pb;
